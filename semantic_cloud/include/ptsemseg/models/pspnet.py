...
if __name__ == '__main__':
    ...
    from ptsemseg.loader.pascal_voc_loader import pascalVOCLoader as cl
    psp = pspnet(version='ade20k')

    # Just need to do this one time
    caffemodel_dir_path = '/home/interns/xuan/models'
    ...
    #psp.load_state_dict(convert_state_dict(torch.load('/home/interns/xuan/models/pspnet_101_pascalvoc.pth')['model_state']))

    psp.float()
    psp.cuda(cd)
    psp.eval()

    dataset_root_dir = '/home/interns/xuan/datasets/VOC/VOCdevkit/VOC2012'
    dst = cl(root=dataset_root_dir, is_transform=True)
    img = m.imread(os.path.join(dataset_root_dir, 'JPEGImages/2007_000033.jpg'))
    m.imsave('cropped.jpg', img)

    orig_size = img.shape[:-1]
    #img = misc.imresize(img,( 473, 473) ) # uint8 with RGB mode, resize width and height which are odd numbers
    ...
    img = img.unsqueeze(0)
    inp = Variable(img, volatile=True)
    print(inp)
    out = psp(inp.cuda()).data.cpu().numpy()
    #out = F.softmax(psp.forward(inp.cuda()), dim=1).data.cpu().numpy()

    #out = psp.tile_predict(img)
    pred = np.argmax(out, axis=1)[0]

    decoded = dst.decode_segmap(pred)
    m.imsave('2007_000033_pred.jpg', decoded)
    #m.imsave('cityscapes_sttutgart_tiled.png', pred)


    checkpoints_dir_path = '/home/interns/xuan/models'
